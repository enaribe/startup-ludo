...
Le jeu utilise **deux méthodes**, et choisit la meilleure disponible.

### Méthode principale : l'intelligence artificielle

Le jeu demande à une IA d'analyser l'entreprise du joueur. L'IA joue le rôle
d'un **analyste financier spécialisé dans les startups africaines**.

On lui transmet :

- le **nom** de l'entreprise,
- sa **description**,
- la **cible** choisie (le public visé : étudiants, agriculteurs…),
- la **mission** choisie (le problème résolu : éduquer, connecter…),
- le **secteur** d'activité.

À partir de ça, l'IA estime une valorisation et **explique son raisonnement**
en quelques phrases simples, avec une liste de facteurs (par exemple :
« cohérence entre la cible et la mission », « secteur porteur », « originalité
de l'idée »).
...
Résultat : une entreprise bien construite vaut plus qu'une entreprise basique,
même sans l'IA.
...
Le joueur n'a pas à attendre : le jeu affiche **tout de suite** une valorisation
estimée (le calcul automatique), puis la **remplace** par celle de l'IA dès
qu'elle est prête, avec son explication.

Le joueur peut aussi ouvrir une **fenêtre d'explication** qui détaille comment
sa valorisation a été obtenue (la base, et les bonus liés à ses cartes).

---

## Et après la création ?

La valorisation **n'est pas figée**. Après la création, elle **augmente à
chaque partie** jouée avec cette entreprise, en fonction des jetons gagnés.

Le joueur suit l'évolution de la valeur de ses entreprises dans son
**portfolio**.
