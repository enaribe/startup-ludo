/**
 * Types du système Challenge (Programme d'accompagnement)
 */
...
export function getLevelProgress(
  ...
  return Math.min(100, Math.floor((currentXp / levelXpRequired) * 100));
}
...
export function getChallengeProgress(
  ...
  return Math.min(100, Math.floor((totalXp / totalXpRequired) * 100));
}
...
export function isLevelUnlocked(
  ...
  const prevXp = prevLevel ? xpByLevel[prevLevel.number] ?? 0 : 0;
  return prevXp >= (prevLevel?.xpRequired ?? 0);
}
