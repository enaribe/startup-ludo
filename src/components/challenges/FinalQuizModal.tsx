/**
 * Quiz final certification 16 questions, 75% pour réussir
 */
...
import { QUIZ_FINAL_QUESTIONS, QUIZ_PASS_THRESHOLD } from '@/data/challenges';
...
export const FinalQuizModal = memo(function FinalQuizModal({
  ...
  const submitAnswer = useCallback(() => {
    const q = QUIZ_FINAL_QUESTIONS[currentIndex];
    if (!q || selectedOption === null) return;
    const correct = selectedOption === q.correctIndex;
    setScore((s) => s + (correct ? 1 : 0));
    if (hapticsEnabled) {
      if (correct) Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
      else Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
    }
    if (currentIndex < QUIZ_FINAL_QUESTIONS.length - 1) {
      setCurrentIndex((i) => i + 1);
      setSelectedOption(null);
    } else {
      setPhase('result');
    }
  }, [currentIndex, selectedOption, hapticsEnabled]);

  const handleResultAction = useCallback(() => {
    const total = QUIZ_FINAL_QUESTIONS.length;
            ...
            <View style={styles.inner}>
              ...
              {phase === 'result' && (
                <Animated.View entering={FadeInDown.duration(300)}>
                  ...
                  <Text style={styles.resultScore}>Score : {score}/{total} ({(score / total * 100).toFixed(0)}%)</Text>
                  <Text style={styles.resultHint}>
                    {passed ? 'Tu as obtenu le statut Champion Local.' : `Il faut au moins ${Math.ceil(QUIZ_PASS_THRESHOLD * 100)}% pour réussir.`}
                  </Text>
                  <GameButton
                    variant={passed ? 'green' : 'blue'}
                    title={passed ? 'OBTENIR MON CERTIFICAT' : 'RÉESSAYER'}
...
const styles = StyleSheet.create({
  ...
  resultScore: { fontFamily: FONTS.bodyBold, fontSize: FONT_SIZES.lg, color: COLORS.primary, textAlign: 'center', marginBottom: SPACING[2] },
  resultHint: { fontFamily: FONTS.body, fontSize: FONT_SIZES.sm, color: COLORS.textSecondary, textAlign: 'center', marginBottom: SPACING[4] },
});
